...
class ADLSModule:
    ...
    def copy_file_between_container(self, source_container_name, source_file_path, destination_container_name, destination_file_path):
        source_blob_client = self._blob_service_client.get_blob_client(
                container=source_container_name, blob=source_file_path
        )
        destination_blob_client = self._blob_service_client.get_blob_client(
                container=destination_container_name, blob=destination_file_path
        )

        destination_blob_client.start_copy_from_url(source_blob_client.url)
        copy_properties = destination_blob_client.get_blob_properties()
        if copy_properties['copy']['status']=='success':
            print(f"Successfully copied {source_file_path} to {destination_file_path}")
        else:
            print(f"Failed to copy {source_container_name}: {source_file_path} to {destination_container_name}: {destination_file_path}")
    ...
    def read_file_in_path(self, container_name, path):
        if path is None:
            return None

        container_client = self._blob_service_client.get_container_client(container_name)
        try:
            # List blobs with the specified path prefix (single file expected)
            blobs = container_client.list_blobs(name_starts_with=path)
            if not blobs:
                raise FileNotFoundError(f"File '{path}' not found in container '{container_name}'")

            # Assuming only one file matches the path
            blob = next(blobs)
            blob_client = container_client.get_blob_client(blob.name)
            data = blob_client.download_blob().readall()

            return {"name": blob.name, "url": blob_client.url, "data": data}
